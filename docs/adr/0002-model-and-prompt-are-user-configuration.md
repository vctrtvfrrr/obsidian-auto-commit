...
- The *shape* of the contract is instructed, never verified after the fact: there is no parsing that hunts for code fences or preambles. Deciding what to do on that kind of failure — reject, re-ask, rewrite — is not justified for a personal vault.
- **Completeness is verified, because a partial message is indistinguishable from a good one.** `max_tokens` bounds reasoning plus message together, so at higher effort a long thinking pass can leave a truncated message behind, and a refusal arrives as ordinary text. Only a response that finished on its own is accepted (`stop_reason === "end_turn"`, non-empty after trimming); anything else fails the run and leaves the changes staged. This is ADR-0001's refusal to truncate the payload, applied to the other end of the same request: a history that lies is worse than a commit the user has to write by hand.
